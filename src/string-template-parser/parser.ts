...
export function parseStringTemplateGenerator({
  ...
  QUOTED_STRING_IN_PARAMETER_GET_AND_ADVANCE?: (
    remainingString: string,
    advance: (length: number) => void,
  ) => string | null;
} = {}) {
  const quotedStringTest =
    QUOTED_STRING_TEST ||
    ((remainingString: string) => QUOTED_STRING.test(remainingString));
  const quotedStringGetAndAdvance =
    QUOTED_STRING_GET_AND_ADVANCE ||
    getQuotedStringAndAdvanceForRegex(QUOTED_STRING);
  const quotedStringInParameterTest =
    QUOTED_STRING_IN_PARAMETER_TEST ||
    ((remainingString: string) => QUOTED_STRING.test(remainingString));
  const quotedStringInParameterGetAndAdvance =
    QUOTED_STRING_IN_PARAMETER_GET_AND_ADVANCE ||
    getQuotedStringAndAdvanceForRegex(QUOTED_STRING);
      ...
      switch (parseState) {
        ...
        case ParseState.Variable:
          ...
          if (quotedStringTest(remainingString)) {
            currentVariable.name += quotedStringGetAndAdvance(
              remainingString,
              advance,
            );
            continue;
        ...
        case ParseState.Pipe:
          ...
          if (testVariableEnd() || testPipeParameterStart() || testPipeStart())
            continue;
          if (quotedStringTest(remainingString)) {
            currentPipe.name += quotedStringGetAndAdvance(
              remainingString,
              advance,
            );
            continue;
        ...
        case ParseState.PipeParameter:
          ...
          if (quotedStringInParameterTest(remainingString)) {
            currentPipeParameter += quotedStringInParameterGetAndAdvance(
              remainingString,
              advance,
            );
            continue;
